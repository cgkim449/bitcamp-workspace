// 다형성 - 다형적 변수(polymorphic variables) : 그 레퍼런스에 어떤 클래스 객체를 담을 수 있냐의 문제
// - 슈퍼클래스 레퍼런스는 서브클래스 객체를 가리킬 수 있다.

// - 왜? 개념적(계층도), 물리적(하위가 상위의 인스턴스 필드를 모두 포함하니까)으로 가능하다
// - 이러한 v2를 다형적 변수라고 한다(v2 = c : vehicle아 Car을 가리켜라)
