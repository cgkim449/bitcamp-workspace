...
public class Exam02 {
  public static void main(String[] args) throws Exception {
    ...
    System.out.println(s1.compareTo(s2)); // 정렬상 앞에있는문자인지 뒤에있는 문자인지 알고싶을때
    //양수면 뒤에나온다, 음수면 전에나온다
    ...
    System.out.println("-----------------------");
    byte[] bytes = s2.getBytes("MS949"); // JVM ==> 지정한 charset
    // JVM(UTF-16) ==> OS기본문자표(linux/uinx : UTF-8, windows : MS949)

    //"ABC가각"
    // - Linux/Unix : 41 42 43 ea b0 80 ea b0 81
    // - Windows : 41 42 43 b0 a1 b0 a2
    for (byte b : bytes) {
      System.out.println(Integer.toHexString(b & 0xff)); //출력할때는 4바이트로 ??
    }
...
System.out.println("----------------------------------");
    ...
    Integer i3 = Integer.valueOf(100); // 4바이트 정수를 다룰 인스턴스가 생성
