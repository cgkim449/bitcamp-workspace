...
public class Exam0421 {
  static class SmartPhone {
    ...
    // 생성자란?(constructor)
    // - 인스턴스를 만든 후 사용하기전에 적절한 값으로 초기화시킬 필요가 있다
    // - (제품을 만든 후에 제품을 사용하기 전에
    //    제품이 구동되는데 문제가 없도록 기본 값으로 설정되어야한다.)
    ...
    // 인스턴수 변수가 사용되기 전에 적절한 값으로 초기화시키기위해
    // 자동으로 호출하는 메서드
    //
  }

  public static void main(String[] args) {
    // 인스턴스를 생성할 때 반드시 호출될 생성자를 지정해야한다
    // 안하면 문법 오류이다
    ...
    // 다음과 같이 호출될 생성자를 지정하지 않ㅇ느면 문법 오류이다
    //SmartPhone obj1 = new SmartPhone; // 컴파일 오류!

    // 다음과 같이 존재하지 않는 생성자를 지정하면 문법 어류이다.
    //SmartPhone obj1 - new SmartPhone(100); // int 파라미터를 가지는 생성자가 없다.


    // SmartPhone 인스턴스를 만들때 기본 생성자가 호출되게 한다
    // 다음과 같이 기본 생성자를 지정할 수 있다
    // SmartPhone obj1 = new SmartPhone(); // 파라미터가 없는 생성자
     SmartPhone obj1 = new SmartPhone(1); // 파라미터가 없는 생성자
//     SmartPhone obj2 = new SmartPhone(3.14); // 파라미터가 없는 생성자
    obj1.playMusic();
